...
/// </summary>
public sealed class FitFanShopExceptionHandler(
    ILogger<FitFanShopExceptionHandler> logger,
...
) : IExceptionHandler
{
    public async ValueTask<bool> TryHandleAsync(HttpContext ctx, Exception ex, CancellationToken ct)
    {
        ...
        {
            FitFanShopNotFoundException => StatusCodes.Status404NotFound,
            FitFanShopConflictException or FitFanShopBusinessRuleException => StatusCodes.Status409Conflict,
    ...
    private static ErrorDto BuildErrorDto(Exception ex, bool isDev, string traceId)
    {
        ...
        switch (ex)
        {
            case FitFanShopNotFoundException:
            case FitFanShopConflictException:
            case FitFanShopBusinessRuleException:
                code = "entity.error";
                message = ex.Message;
                break;
