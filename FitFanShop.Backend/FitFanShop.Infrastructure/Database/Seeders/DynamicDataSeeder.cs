...
public static class DynamicDataSeeder
{
    ...
    private static async Task SeedOrdersAsync(DatabaseContext context, List<FitFanShopUserEntity> users, List<ProductVariantEntity> variants)
    {
        ...
        {
            new() { UserId = users[1].Id, StatusId = 3, TotalAmount = 299.98m, CreatedAtUtc = DateTime.UtcNow.AddDays(-10) },
            new() { UserId = users[2].Id, StatusId = 2, TotalAmount = 189.99m, CreatedAtUtc = DateTime.UtcNow.AddDays(-5) },
            new() { UserId = users[3].Id, StatusId = 1, TotalAmount = 149.99m, CreatedAtUtc = DateTime.UtcNow.AddDays(-2) }
        };
    ...
    private static async Task SeedTicketsAsync(DatabaseContext context, List<FitFanShopUserEntity> users, List<EventEntity> events, List<TicketTypeEntity> ticketTypes)
    {
        ...
        {
            new() { TicketTypeId = ticketTypes[1].Id, EventId = events[0].Id, UserId = users[1].Id, PricePaid = 80m, QRCode = $"QR-FCFIT-{Guid.NewGuid():N}", SeatNumber = "N-15", CreatedAtUtc = DateTime.UtcNow.AddDays(-7) },
            new() { TicketTypeId = ticketTypes[1].Id, EventId = events[0].Id, UserId = users[1].Id, PricePaid = 80m, QRCode = $"QR-FCFIT-{Guid.NewGuid():N}", SeatNumber = "N-16", CreatedAtUtc = DateTime.UtcNow.AddDays(-7) },
            new() { TicketTypeId = ticketTypes[5].Id, EventId = events[1].Id, UserId = users[2].Id, PricePaid = 120m, QRCode = $"QR-FCFIT-{Guid.NewGuid():N}", SeatNumber = "VIP-A12", CreatedAtUtc = DateTime.UtcNow.AddDays(-3) },
            new() { TicketTypeId = ticketTypes[13].Id, EventId = events[2].Id, UserId = users[3].Id, PricePaid = 70m, QRCode = $"QR-FCFIT-{Guid.NewGuid():N}", SeatNumber = "E-28", CreatedAtUtc = DateTime.UtcNow.AddDays(-1) },
            new() { TicketTypeId = ticketTypes[13].Id, EventId = events[2].Id, UserId = users[3].Id, PricePaid = 70m, QRCode = $"QR-FCFIT-{Guid.NewGuid():N}", SeatNumber = "E-29", CreatedAtUtc = DateTime.UtcNow.AddDays(-1) }
        };
